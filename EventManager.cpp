...
namespace MyEventManager {
	...
	//------
	//Загружать различные файлы настроек .cfg в зависимости
	//от положения игры
	
	//Файл .cfg выглядит так:
	//EVENT_NAME EVENT_TYPE:KEY_VALUE
	void EventManager::LoadBindings(){
		...
		while (std::getline(bindings, line)){
			...
			bind = new Binding(callbackName);
			if (bind == nullptr) {
				throw Exception(ExcType::badAlloc);
			}
			
			while (!keystream.eof()){
				string keyval;
				//считываем пользовательское название события
				keystream >> keyval;
				size_t start = 0;
			...
			for (auto &e_itr : bind->m_events){
				...
				if (sfmlEvent == EventType::KeyDown || sfmlEvent == EventType::KeyUp){
					if (e_itr.second.m_code == l_event.key.code){
						...
						//очень странное условие: у нас изначально m_details.m_keyCode == 1 =>
						//операция внутри if никогда не исполнится
