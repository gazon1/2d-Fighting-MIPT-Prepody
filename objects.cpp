...
namespace MyObjects
{
	...
	Object * ArrayOfObjects::operator[](int index)
	{
		if(isOutOfRange(index))
			throw Exception(ExcType::indexOutOfRange);
		else
	...
	{
		if(isFull())
			throw Exception(ExcType::overflow);

		if(!object)
			throw Exception(ExcType::nullPtr);
	...
	void ArrayOfObjects::deleteObject(Object * object)
	{
		...
		for(auto iter = begin(); iter != end(); ++iter)
		{
			...
			if(currentObject == object)
			{
				...
				//‘MyObject::Object’ which has non-virtual 
		...
		throw Exception(ExcType::elemNotFound);
	};
	...
	{
		if(isLast())
			throw Exception(ExcType::iterOutOfRange);
